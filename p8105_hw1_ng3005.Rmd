...
The `as.numeric` function assigns characters to numeric values. As a result, we see that the variable `vec_positive` with output "values" of `TRUE` and `FALSE` now have output values of `1` and `2`. Similarly, the variable `vec_factor`, which contains 3 levels (`level_1`, `level_2`, `level_3`), now yields outputs `1`, `2`, and `3`, corresponding to each of the levels. On the other hand, the characters within `vec_char` have no numerical association in the same way that those in `vec_factor` do (thanks to the `factor()` function), so `as.numeric` is unable to translate `vec_char` into numeric values. 
